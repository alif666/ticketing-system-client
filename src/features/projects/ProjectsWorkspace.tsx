...
export function ProjectsWorkspace() {
  ...
  function handleModuleError(exception: unknown) {
    setActionError(
      exception instanceof Error ? exception.message : "Unable to save module",
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="mt-1 text-xl font-semibold">Your workspaces</h2>
        </div>
        {canManageProjects && (
      ...
      <FormDialog
        ...
        description={
          editingProject
            ...
            : "Set up a project workspace for your support team."
        }
      >
        <ProjectForm
          project={editingProject ? selectedProject : undefined}
          onSave={async (name, shortCode, description, active) => {
            if (editingProject && selectedProjectId)
              await updateProject(selectedProjectId, name, description, active);
            else await createProject(name, shortCode, description);
          }}
      ...
      <FormDialog
        ...
        onOpenChange={(open) => {
          if (!open) closeModuleForm();
        }}
        title={typeof moduleForm === "number" ? "Edit module" : "Create module"}
        description={
          selectedProject
            ? `Organize ${selectedProject.name} with functional modules.`
            : "Add a functional module."
        }
      >
        <ModuleForm
          module={
            typeof moduleForm === "number"
              ? modules.find((module) => module.id === moduleForm)
              : undefined
          }
          onSave={async (name, description, active) => {
            try {
              if (typeof moduleForm === "number")
                await updateModule(moduleForm, name, description, active);
              else await createModule(name, description);
              closeModuleForm();
              setActionError("");
            } catch (exception) {
              handleModuleError(exception);
      ...
      {!projects.length ? (
        <Card>
          <CardContent className="py-10 text-center">
            ...
            <p className="mt-3 font-medium">No projects available</p>
            <p className="mt-1 text-sm text-muted-foreground">
              You will see projects here when you are granted access.
            </p>
          </CardContent>
        </Card>
      ) : (
